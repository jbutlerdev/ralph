...
export function validateRalphPlan(plan: RalphPlan): {
  ...
  plan.tasks.forEach((task) => {
    task.dependencies.forEach((depId) => {
      // task-000 is a special marker for "no dependencies"
      if (depId === 'task-000') return;
...
export function planFromMarkdown(markdown: string): RalphPlan {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Two formats: "**Description:** Content" OR "**Description:" on its own line
    const descMatch = line.match(/\*\*Description:\*\*/i);
    if (descMatch) {
      // Check if description content is on the same line
      const sameLineContent = line.replace(/\*\*Description:\*\*\s*/i, '').trim();
      currentTask.description = sameLineContent || '';
...
export function getNextTask(plan: RalphPlan, completedTaskIds: Set<string>): RalphTask | null {
  for (const task of plan.tasks) {
    ...
    const dependenciesMet = task.dependencies.every(dep =>
      dep === 'task-000' || completedTaskIds.has(dep)
    );
 ...
 */
export function getPlanPath(planName: string): {
  mdPath: string;
  jsonPath: string;
  dir: string;
} {
 ...
 */
export async function listAllPlans(): Promise<string[]> {
  try {
 ...
 */
export async function loadPlan(planName: string): Promise<RalphPlan> {
  const fs = await import('fs/promises');
 ...
 */
export async function savePlan(planName: string, plan: RalphPlan): Promise<void> {
  const fs = await import('fs/promises');
  const path = await import('path');
